...
def predict_pose(model, labels, landmarks):
    ...
    """
    lst = [(landmark.x - landmarks[0].x, landmark.y - landmarks[0].y) for landmark in landmarks]
...
def main():
    ...
    while True:
        ...
        if results.pose_landmarks and inFrame(results.pose_landmarks.landmark):
            pred, confidence = predict_pose(model, labels, results.pose_landmarks.landmark)
            if confidence > 0.75:
                cv2.putText(frame, pred, (180, 180), cv2.FONT_ITALIC, 1.3, (0, 255, 0), 2)
            else:
                cv2.putText(frame, "Asana is either wrong or not trained", (100, 180), cv2.FONT_ITALIC, 1.8, (0, 0, 255), 3)
        else:
